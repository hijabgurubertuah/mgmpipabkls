...
export const AdminEmbedsTab: React.FC<AdminEmbedsTabProps> = ({ config, onChange }) => {
  ...
  const defaultLocationName = identity.name ? `${identity.name}, Bengkalis` : 'Kecamatan Bengkalis, Riau';
  ...
  const [searchLocation, setSearchLocation] = useState<string>(
    initialDetails.query || defaultLocationName
  );
  const [previewMapUrl, setPreviewMapUrl] = useState<string>(
    embeds.mapIframeUrl || buildGoogleMapsEmbedUrl(defaultLocationName, 17)
  );
  ...
  const handleReset = () => {
    const defaultQuery = defaultLocationName;
          ...
          <div className="sm:col-span-8">
            ...
            <div className="flex gap-2">
              <input
                ...
                className="w-full px-3 py-2 rounded-lg border border-slate-300 text-xs focus:ring-2 focus:ring-emerald-600 focus:outline-none"
                placeholder="MGMP IPA Kecamatan Bengkalis..."
              />
              <button
                type="button"
