...
export const Navbar: React.FC<NavbarProps> = ({
  ...
  return (
    <>
      ...
      {mobileMenuOpen && (
        <div
          ...
          className="fixed inset-0 bg-slate-900/50 backdrop-blur-xs z-[90] lg:hidden animate-in fade-in duration-200"
          aria-hidden="true"
        />
      )}
          ...
          >
            <div className="relative w-9 h-9 sm:w-11 sm:h-11 rounded-lg sm:rounded-xl overflow-hidden bg-white border border-slate-200/90 flex items-center justify-center shrink-0 shadow-xs p-0.5 sm:p-1 group-hover:scale-105 transition-transform">
              {identity.logoUrl ? (
                ...
                <FlaskConical className="w-6 h-6 sm:w-7 sm:h-7 text-blue-700" />
              )}
            </div>
    ...
    {/* Fullscreen Hard Reset / Refresh Loading Screen */}
    {(isHardResetting || isRefreshing) && (
      <div className="fixed inset-0 z-[100] bg-slate-950/80 backdrop-blur-sm flex flex-col items-center justify-center p-6 text-white text-center animate-in fade-in duration-200">
        ...
        <div className="flex items-center gap-3 mb-1.5 justify-center">
          ...
          <h3 className="text-xl sm:text-2xl font-black text-white tracking-wider animate-pulse">
            MGMP IPA KEC. BENGKALIS
          </h3>
        </div>
        <p className="text-xs sm:text-sm text-slate-300 font-medium">
          Memuat ulang data & menyelaraskan konten...
        </p>
      </div>
    )}
    </>
