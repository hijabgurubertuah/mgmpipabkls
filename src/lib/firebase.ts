...
export function normalizeSchoolConfig(raw: Partial<SchoolConfig> | null | undefined): SchoolConfig {
  ...
  // Sanitize legacy school placeholders so they are converted to MGMP IPA Kecamatan Bengkalis
  ...
  if (sanitizedRaw.principal) {
    ...
    if (!p.name || p.name.includes('Syahrul') || p.name === 'Drs. H. Syahrul, M.Pd.' || p.name === 'Kepala Sekolah') {
      p.name = DEFAULT_SCHOOL_CONFIG.principal.name;
    }
    if (!p.title || p.title === 'Kepala Sekolah' || p.title.includes('Kepala SMP')) {
      p.title = DEFAULT_SCHOOL_CONFIG.principal.title;
    }
    if (!p.quote) {
      p.quote = DEFAULT_SCHOOL_CONFIG.principal.quote;
    }
    if (!p.fullSpeech) {
      p.fullSpeech = DEFAULT_SCHOOL_CONFIG.principal.fullSpeech;
    }
  ...
  if (sanitizedRaw.identity) {
    ...
    if (id.logoUrl && (id.logoUrl.includes('photo-1594608661623') || id.logoUrl.includes('logo-smpn-1-bengkalis'))) {
      id.logoUrl = '';
    }
    if (id.faviconUrl && (id.faviconUrl.includes('photo-1546410531') || id.faviconUrl.includes('logo-smpn-1-bengkalis'))) {
      id.faviconUrl = '';
    }
    if (!id.name || id.name.includes('SMP Negeri 1') || id.name.includes('SMPN 1') || id.name === 'Portal Resmi') {
      id.name = DEFAULT_SCHOOL_CONFIG.identity.name;
      id.shortName = DEFAULT_SCHOOL_CONFIG.identity.shortName;
      id.tagline = DEFAULT_SCHOOL_CONFIG.identity.tagline;
      id.tickerText = DEFAULT_SCHOOL_CONFIG.identity.tickerText;
      id.tickerEnabled = true;
      id.accreditationTickerText = DEFAULT_SCHOOL_CONFIG.identity.accreditationTickerText;
      id.accreditationTickerEnabled = true;
    }
    sanitizedRaw.identity = id;
  }

  if (sanitizedRaw.header) {
    const h = { ...sanitizedRaw.header };
    if (!h.heroTitle || h.heroTitle.includes('SMP Negeri 1') || h.heroTitle.includes('SMPN 1')) {
      h.heroTitle = DEFAULT_SCHOOL_CONFIG.header.heroTitle;
      h.heroSubtitle = DEFAULT_SCHOOL_CONFIG.header.heroSubtitle;
      h.highlights = DEFAULT_SCHOOL_CONFIG.header.highlights;
      h.heroImageUrl = DEFAULT_SCHOOL_CONFIG.header.heroImageUrl;
    }
    sanitizedRaw.header = h;
  }

  if (sanitizedRaw.footer) {
    const f = { ...sanitizedRaw.footer };
    if (!f.email || f.email.includes('smpn1') || f.email.includes('sekolah')) {
      f.email = DEFAULT_SCHOOL_CONFIG.footer.email;
    }
    if (!f.aboutText || f.aboutText.includes('SMP Negeri 1')) {
      f.aboutText = DEFAULT_SCHOOL_CONFIG.footer.aboutText;
    }
    if (!f.copyright || f.copyright.includes('SMP Negeri 1')) {
      f.copyright = DEFAULT_SCHOOL_CONFIG.footer.copyright;
    }
  ...
  return {
    ...
    agendas: Array.isArray(sanitizedRaw.agendas) && sanitizedRaw.agendas.length > 0 ? sanitizedRaw.agendas : DEFAULT_SCHOOL_CONFIG.agendas,
  };
}
