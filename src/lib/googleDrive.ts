...
async function getOrCreateSchoolFolder(token: string): Promise<string | null> {
  const folderName = '[MGMP IPA Bengkalis] Web Assets';
  try {
    ...
    const createRes = await fetch('https://www.googleapis.com/drive/v3/files', {
      ...
      body: JSON.stringify({
        ...
        description: 'Folder penyimpanan otomatis foto website MGMP IPA Kecamatan Bengkalis',
      }),
    });
...
export async function uploadImageToDrive(
  ...
  const metadata = {
    ...
    description: 'Diunggah melalui Admin CMS MGMP IPA Kecamatan Bengkalis',
    parents: folderId ? [folderId] : undefined,
  };

  const boundary = 'MGMP_IPA_BENGKALIS_DRIVE_UPLOAD_' + Date.now();
