...
export function syncPWAManifest(identity: SchoolIdentity) {
  ...
  const schoolName = identity.name || 'MGMP IPA Kecamatan Bengkalis';
  const shortName = identity.shortName || identity.name || 'MGMP IPA';
  const DEFAULT_LOGO_URL = '/icon.svg';
  const rawLogo = identity.logoUrl || '';
  const cleanLogo = (rawLogo && !rawLogo.includes('photo-1594608661623') && !rawLogo.includes('logo-smpn-1-bengkalis')) ? rawLogo : '';
  const rawFavicon = identity.faviconUrl || identity.logoUrl || '';
  const cleanFavicon = (rawFavicon && !rawFavicon.includes('photo-1546410531') && !rawFavicon.includes('photo-1594608661623') && !rawFavicon.includes('logo-smpn-1-bengkalis')) ? rawFavicon : '';
